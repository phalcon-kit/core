-- Core 4.0 fresh-install baseline. References use the selected database.
CREATE TABLE `file` (
    `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    `uuid` CHAR(36) CHARACTER SET ascii COLLATE ascii_general_ci NOT NULL,
    `label` VARCHAR(255) NULL COMMENT 'The original or display name of the file',
    `category` ENUM('other') NOT NULL DEFAULT 'other',
    `path` VARCHAR(255) NOT NULL COMMENT 'Relative path to the file in storage',
    `mime_type` VARCHAR(100) NULL COMMENT 'e.g., image/jpeg, application/pdf',
    `extension` VARCHAR(20) NULL COMMENT 'e.g., jpg, pdf',
    `size` BIGINT UNSIGNED NULL COMMENT 'Size in bytes',
    `deleted` TINYINT(1) UNSIGNED NOT NULL DEFAULT '0',
    `created_at` DATETIME NOT NULL DEFAULT current_timestamp(),
    `created_by` BIGINT UNSIGNED NULL,
    `updated_at` DATETIME NULL,
    `updated_by` BIGINT UNSIGNED NULL,
    `deleted_at` DATETIME NULL,
    `deleted_by` BIGINT UNSIGNED NULL,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uuid_UNIQUE` (`uuid`),
    KEY `idx_category` (`category`),
    KEY `idx_mime_type` (`mime_type`),
    KEY `idx_extension` (`extension`),
    KEY `fk_file_created_by` (`created_by`),
    KEY `fk_file_updated_by` (`updated_by`),
    KEY `fk_file_deleted_by` (`deleted_by`),
    CONSTRAINT `fk_file_created_by` FOREIGN KEY (`created_by`) REFERENCES `user`(`id`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `fk_file_deleted_by` FOREIGN KEY (`deleted_by`) REFERENCES `user`(`id`) ON DELETE SET NULL ON UPDATE CASCADE,
    CONSTRAINT `fk_file_updated_by` FOREIGN KEY (`updated_by`) REFERENCES `user`(`id`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
