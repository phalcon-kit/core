...
Use this guide when upgrading an older Zemit/Phalcon Kit 0.x application to the
...
| --- | --- |
| Phalcon DevTools model generator | Phalcon Kit database-first scaffolder |
...
- The Phalcon baseline moved from the older 5.9.x package surface toward the
  current supported Phalcon surface.
...
3. Run the Phalcon Kit scaffolder.
...
controller style. Phalcon Kit core already keeps some narrow deprecated action
...
only generic, tested, framework-level compatibility into Phalcon Kit core.
...
In 1.x, extend the Phalcon Kit API controller, initialize the temporary bridge
...
- shared permission helpers return condition blocks, not raw SQL plus global
  bind mutation;
- models are generated by Phalcon Kit scaffolding;
